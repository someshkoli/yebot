...
def index(request):
	...
	if data['message']['text']=="/get_man":
		get_man(request)
	elif data['message']['text']=="/get_assign":
		get_assig(request)
	else:
		false_request()
...
def get_man(request):
	...
	return requests.post('https://api.telegram.org/bot727019423:AAEnYQDHDKygojIJovPpAttzskvS6bAaB5g/sendMessage',
							json={'chat_id':573080922,'text':"select subject for manual",
									'reply_markup':{'inline_keyboard':[
																[{'text':'ostl','url':'www.google.com'}],
																[{'text':'am','url':'www.google.com'}],
																[{'text':'coa','url':'www.google.com'}],
																[{'text':'aoa','url':'www.google.com'}]
															]
											}
									}
							)
